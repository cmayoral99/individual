...
class TurtleRotationProportionalControl:
    ...
    def rotate_turtle_to_target(self, target_theta_deg: float):
        Kp = 5.0  # Ganancia proporcional menor para giro más suave
        max_speed = 1.0  # Velocidad angular máxima en rad/s
        ...
        while not rospy.is_shutdown():
            ...
            # Limitar la velocidad angular para evitar giros bruscos
            if vel_z > max_speed:
                vel_z = max_speed
            elif vel_z < -max_speed:
                vel_z = -max_speed
            ...
            rospy.loginfo(f"Error de ángulo: {degrees(error):.2f}°, Velocidad angular: {vel_z:.2f} rad/s")
